...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
	...
	// To find all the income or cost have been does by a user in a single currency at a time
	List<Transaction> findByTimeAndCurrencyAndUserAndType(LocalDateTime time, Currency currency, User user,
			TransactionType type);

	// To find all the income or cost have been does by a user in a single currency between a time range
	List<Transaction> findByTimeBetweenAndCurrencyAndUserAndType(LocalDateTime time1, LocalDateTime time2,
			Currency currency, User user, TransactionType type);
